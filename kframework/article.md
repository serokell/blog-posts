...
* `MonadSMT m` is a typeclass that allows accessing `SMT` through monad transformers. It has two base instances: `MonadSMT SMT` and `MonadSMT NoSMT`.
* `SimplifierT smt a` is a datatype representing a simplification action, the `smt` type variable can be subsittuted with either `SMT` or `NoSMT`.
...
Turned out, the `simplifier` type variable is instantiated to different monad transformer stacks and could not be easily monomorphized in some functions. We developed a custom tool, `Debug.Trace.Usage`, in order to determine how the monad is being instantiated in practice. Here is the summary of its results on one of the test cases:
